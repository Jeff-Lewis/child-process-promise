...
var q = require('q');
var debug = require('debug')('scm:dep:child-process-spawn');
...
function spawn(command, args, options) {
    ...
    debug(util.format('spawning with command "%s"', command));
    debug('spawning args and options:', args, options);
    ...
    cp.on('close', function (code) {
        if (code !== 0) {
            var commandStr = command + (args && args.length ? (' ' + args.join(' ')) : '');
            var err = new Error('`' + commandStr + '` failed with code ' + code, code);
